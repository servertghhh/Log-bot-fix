...
# ============ ডাটা পার্সার ============
def extract_all_credentials(text):
    lines = text.split('\n')
    all_credentials = []
                        ...
                        for item in items:
                            if isinstance(item, str):
                                ...
                                if item.startswith('Username:'):
                                    current_username = item.replace('Username:', '').strip()
                                elif item.startswith('Pass:'):
                                    current_password = item.replace('Pass:', '').strip()
                                elif item.startswith('Url:'):
                                    current_url = item.replace('Url:', '').strip()
...
def callback_handler(call):
    ...
    if call.data == "verify_me":
        if check_channel_membership(user_id):
            ...
            bot.answer_callback_query(call.id, "আগে চ্যানেলে জয়েন কর", show_alert=True)
